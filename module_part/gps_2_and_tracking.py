...
def process_frame(frame):
    res = model.track(frame, persist=True, conf=0.5)
    ...
    while True:
        ...
        if frame_count % 2 != 0:
            ...
            if len(res[0].boxes) > 0:
                ...
                if new_ids:
                    with lock:
                        current_gps = last_gps

                    if current_gps:
                        lat, lat_dir, lon, lon_dir = current_gps
                        ...
                        for box in res[0].boxes:
                            ...
                            track_id = int(box.id.item()) if box.id is not None else "N/A"
                            detection_details.append(f"{cls_name} ID:{track_id} ({conf:.2f})")
